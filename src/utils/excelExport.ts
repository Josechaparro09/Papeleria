 ...
 */
const prepareProductsData = (sales: Sale[], products: Product[]) => {
 ...
 */
const prepareServicesData = (sales: Sale[], services: Service[]) => {
 ...
 */
const preparePrintingData = (records: PrintingRecord[]) => {
 ...
 */
const prepareSummaryData = (productsData: any[], servicesData: any[], printingData: any[]) => {
...
export const exportPrintingProfitSummaryToExcel = (
  ...
  try {
    ...
    const summaryData = Object.entries(monthlyData).map(([yearMonth, data]) => {
      ...
      const formattedMonth = format(new Date(parseInt(year), parseInt(month) - 1, 1), 'MMMM yyyy');
      ...
      // Ordenar por fecha descendente (necesitamos convertir mes-año a una fecha para comparar)
